...
public class HealthAbility extends StatBoostAbility {
    ...
    public void addBoost(Player p, Ability ability) {
        ...
        if(user == null)
            return;
        SmallPetsCommons.getSmallPetsCommons().getHealthModifierUtils().addModifier(p, getID(), statBoostAbility.getStatBoost(user.getSelected().getLevel()), AttributeModifier.Operation.ADD_NUMBER);
